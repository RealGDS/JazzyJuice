...
function reloadall() {
  document.getElementById("click").innerHTML =
    "JJ/click: " + addcomma(moneyup) + " | JJ/sec: " + addcomma(msec);
  document.getElementById("total").innerHTML = "JJ: " + addcomma(money);
  document.getElementById("jjmachine").innerHTML =
    catown + "x Jazzy Juice Machine: " + addcomma(catcost) + " | +" + addcomma(catadd) + "/sec";
  document.getElementById("slave").innerHTML =
    workerown + "x Marcus Slave: " + addcomma(workercost) + " | +" + addcomma(workadd) + "/sec";
  document.getElementById("morejj").innerHTML =
    addcomma(upown) + "x More Jazzy Juice: " + addcomma(upcost);
}
...
function upgrade(name) {
  if (name == "Jazzy Juice Machine") {
    if (money >= catcost && catown < 50) {
      ...
      catcost = catcost * 2;
      document.getElementById("jjmachine").innerHTML =
        catown + "x Jazzy Juice Machine: " + addcomma(catcost) + " | +" + addcomma(catadd * cboost) + "/sec";
    } else if (catown == 50) {
      document.getElementById("jjmachine").innerHTML =
        catown + "Jazzy Juice Machine: MAX | +15% click/sec";
    }
  }

  if (name == "Marcus Slave") {
    if (money >= workercost && workerown < 50) {
      ...
      document.getElementById("slave").innerHTML = 
        workerown + "x Marcus Slave: " + addcomma(workercost) + " | +" + addcomma(workadd * wboost) + "/sec";
    } else if (workerown == 50) {
      document.getElementById("slave").innerHTML =
        workerown + "Marcus Slave: MAX | +35% click/sec";
    }
  }

  if (name == "More Jazzy Juice") {
    if (money >= upcost) {
      moneyup += upcost / 15;
      money -= upcost;
      upown += 1;
